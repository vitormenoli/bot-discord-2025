...
module.exports = {
    ...
    run: async(client, interaction) => {
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .setDescription(`> 🔎 Você tem certeza que deseja desbanir **${member.username}** do servidor?\n\n> 📔 **Motivo:** ${motivo}`)

        const msg = await interaction.reply({ embeds: [embed], components: [row] })

        const filter = i => i.user.id === interaction.user.id && (i.customId === 'confirm_unban' || i.customId === 'cancel_unban')
        ...
        collector.on('collect', async(i) => {
            ...
            if (i.customId === 'confirm_unban') {
                try {
                    ...
                    embed.setTitle('Desbanimento Confirmado')
                         .setDescription(`> ✅ O usuário **${member.username}** foi desbanido com sucesso!\n\n> 📔 **Motivo:** ${motivo}`)

                    interaction.editReply({ embeds: [embed], components: [] })
                } catch (error) {
                    embed.setColor('Red')
                         .setTitle('Erro ao Desbanir Usuário')
                         .setDescription(`❌ Não foi possível desbanir o usuário **${member.username}**. Verifique minha permissão e a hierarquia de cargos.`)
                         interaction.editReply({ embeds: [embed], components: [] })
                }
            } else {
                embed.setColor('Red')
                     .setTitle('Desbanimento Cancelado')
                     .setDescription(`> ❌ O desbanimento do usuário **${member.username}** foi cancelado.`)
                     interaction.editReply({ embeds: [embed], components: [] })
            }
        })
